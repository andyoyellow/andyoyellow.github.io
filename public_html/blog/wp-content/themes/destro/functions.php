...
function Destro_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 )
		$title .= " $sep " . sprintf( __( 'Page %s', 'destro' ), max( $paged, $page ) );
...
class Destro_sidebarads extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			'300x250', // Base ID
			__('300x250 Ads', 'destro'), // Name
			array( 'description' => __( 'widget that displays 300x250 ads in sidebar', 'destro' ), ) // Args
		);
	}
...
class Destro_sidebarads_onetwofive extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			'125x125', // Base ID
			__('125x125 Ads', 'destro'), // Name
			array( 'description' => __( 'widget that displays 125x125 ads in sidebar', 'destro' ), ) // Args
		);
	}	
...
function destro_upgrade(){
	
	wp_enqueue_script('Destro_upgrade', get_template_directory_uri().'/js/destro-upgrade.js' );
	
	echo '<script type="text/javascript">';
	
		echo 'var destroThemeName = "'.wp_get_theme().'";';
		echo 'var destroUpgradeImage = "'.get_template_directory_uri().'/images/upgrade.png'.'";';
	
	echo '</script>';
	
}

add_action('customize_controls_print_scripts', 'destro_upgrade');